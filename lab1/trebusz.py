...
# def calculate_shot_distance(v0: int, h: int, angle: float) -> float:
#     angle_rad = math.radians(angle)
#     return v0 * math.cos(angle_rad) * ((v0 * math.sin(angle_rad) + math.sqrt((v0 * math.sin(angle_rad))**2 + 2 * GRAVITY * h)) / GRAVITY)

# def was_target_hit(target_distance: int, shot_distance: float) -> bool:
#     return abs(target_distance - shot_distance) <= MARGIN_OF_ERROR

# def plot_trajectory(angle_deg: float, target_distance: int) -> None:
#     v0 = INITIAL_VELOCITY
#     h = HEIGHT

#     angle_rad = math.radians(angle_deg)
#     def f(x):
#         return (-GRAVITY / (2 * v0 ** 2 * math.cos(angle_rad) ** 2)) * x ** 2 + (math.sin(angle_rad) / math.cos(angle_rad)) * x + h
    
#     x_values = range(0, 350, 5)
#     y_values = [f(x) for x in x_values]

#     plt.plot(x_values, y_values, color='blue')
#     plt.scatter(target_distance, 0, color='red', label='Target')
#     plt.title('Function Plot')
#     plt.xlabel('X')
#     plt.ylabel('Function Line')
#     plt.grid(True)
